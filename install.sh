...
fimenu() {
    ...
    if [[ "${STATUS[0]}" =~ $(text 27)|$(text 28) ]]; then
        hint " 1.  $(text 29)"
        [ "${STATUS[0]}" = "$(text 28)" ] && hint " 2.  $(text 27) Sing-box (sb -s)" || hint " 2.  $(text 28) Sing-box (sb -s)"
        [ "${STATUS[1]}" = "$(text 28)" ] && hint " 3.  $(text 27) Argo (sb -a)" || hint " 3.  $(text 28) Argo (sb -a)"
        hint " 4.  $(text 92)"
        hint " 5.  $(text 30)"
        hint " 6.  $(text 31)"
        hint " 7.  $(text 62)"
    ...
    if [[ "${STATUS[0]}" =~ $(text 27)|$(text 28) ]]; then
        case "$CHOOSE" in
            ...
            3)
                ...
                ;;
            4) change_argo; exit 0 ;;
            5) change_start_port; exit 0 ;;
            6) version; exit 0 ;;
            7) change_protocols; exit 0 ;;
            8) change_cdn; exit 0 ;;
            9) enable_bbr ;;
            10) disable_bbr ;;
            11) uninstall; exit 0 ;;
            0) exit 0 ;;
            *) warning " $(text 36) [0-11] " && sleep 1 && menu ;;
        esac
    else
        case "$CHOOSE" in
            1) IS_FAST_INSTALL='is_fast_install'; CHOOSE_PROTOCOLS='a'; START_PORT="$START_PORT_DEFAULT"; CDN="${CDN_DOMAIN[0]}"; IS_SUB='is_sub'; IS_ARGO='is_argo'; PORT_HOPPING_RANGE='50000:51000'; install_sing-box; export_list install; create_shortcut; exit 0 ;;
            0) exit 0 ;;
            *) warning " $(text 36) [0-1] " && sleep 1 && menu ;;
        esac
    fi
}
