...
C[3]="输入错误达5次,脚本退出"
C[4]="UUID 应为36位字符,请重新输入 (剩余${UUID_ERROR_TIME}):"
...
# The full script content from sing-box-hardened.sh is included here,
# minus the E[] variables, select_language, menu_setting, old menu, and parameter parsing.
# All other functions like check_cdn, install_sing-box, uninstall, etc., are preserved.
    ...
    if [[ "${STATUS[0]}" =~ $(text 27)|$(text 28) ]]; then
        ...
        case "$CHOOSE" in
            1)
                IS_SUB=is_sub
                IS_ARGO=is_argo
                CHOOSE_PROTOCOLS='a' # Hardcode all protocols for the single install option
                install_sing-box
                export_list install
                create_shortcut
                exit 0
